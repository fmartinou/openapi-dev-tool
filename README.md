...
[![Build Status](https://img.shields.io/travis/lyra/openapi-dev-tool/master)](https://travis-ci.org/lyra/openapi-dev-tool)
...
Although there are a lot of advantages to use OpenAPI like documentation & code generation, publish your API faster, promote your teams collaboration, etc., industrialize it remains complicated due to certain shortcomings. Using a same OpenAPI file into several contexts is not supported by the current release of OpenAPI (to propose a same API but which could be used in public and private use). Using the external references to design your API from several files to facilitate the maintainability is still involved. Painlessly publish your specifications into a software repository to be able to reuse it (to generate client or server code). Etc.
Also, the OpenAPI ecosystem is very large (https://openapi.tools/) with a lot of tools with different quality level. Consequently, to set up a complete development environment, many tools has to be installed and upgraded regularly.

OpenAPI Dev Tool corrects these disadvantages by proposing to developers a unique tool to address development and industrialization needs!
...
- write your OpenAPI specifications by using several files (to avoid having a unique large file unmaintainable) ;
- publish easily your OpenAPI files and reuse it to generate your client or your server ;
...
- **Bundle OpenAPI files** to merge the whole of files before serving. By using [OpenAPI remote reference](https://swagger.io/docs/specification/using-ref/), you can work on several files. That facilitates the maintainability and avoid having a large OpenAPI file!
...
- **`x-tags` vendor extension** in `Info` object to be able to [tag OpenAPI files](#tags-usage). OpenAPI doesn't support to tag a specification. Just tag (to organize) operations is possible. With `x-tags` vendor extension and for Redoc viewer, it is now possible to categorize and organize specifications

For example, declare a private and a public specification:
...
> Use the version and title of your specifications to publish it directly in software repository server!

- **Publish one or several OpenAPI files** specified in [configuration file](#configuration-file) and written in YAML or JSON
- **Bundle OpenAPI files** to merge the whole of files before publishing. By using (OpenAPI remote reference)[https://swagger.io/docs/specification/using-ref/], you can work on several files. That facilitates the maintainability and avoid having a large OpenAPI file!
- **Context uses**: it is possible to serve a same specification in [several contexts](#context-usage) (internal, public, etc)
...
> **Warning**
>
> To publish your OpenAPI files, `curl` command is required on host where `openapi-dev-tool` is executed.
> `curl` is available under unix systems and Windows 10, however, for Windows < 10, you can download it from https://curl.haxx.se/windows/.
...
openapi-dev-tool publish
  ...
  Publish into a software repository server (like Sonartype Nexus) one or serveral OpenAPI specifications
  files
...
```json
{
    "folder": "./specs",  // Root folder where are stored specifications
    "specs": [            // Array of specifications (several specifications can be exposed)
        { // First specification file
            "file": "/petstore.yaml", // Relative path of main file of the specification (from "folder" parameter). Has to be an OpenAPI file in YAML or JSON
...
By default, serve feature renders HTML content by parsing EJS templates defined in `openapi-dev-tool` module:

- `api.ejs`: for API List page. This page lists the whole of available APIs before accessing it with Redoc viewer
...
- `api.ejs`
  - `specs`: array of OpenAPI specifications where each `spec` item contains the following attributes:
    ...
    - `tags`: tags loaded by using x-tag vendor extension
- `redoc.ejs`
...
To do that, you can use EJS template directly in your OpenAPI files to specify different behaviors where `context` specified in Configuration file will be used for rendering. For example for public accesses, we want to add a security layer which doesn't exist for internal accesses:
...
This specification could be used in several contexts (public & internal):
...
In OpenAPI specifications, it is possible to use `Tag` object to group operations in several categories. It is great to organize our specifications but it is not usable to tag the whole of OpenAPI file. For example to indicate that the current specification is public, internal, etc.
